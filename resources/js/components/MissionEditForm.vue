    ...
    <div class="container-fluid mt-4 px-4">
        ...
        <form class="mt-3" @submit.prevent="submitForm" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <div v-for="media in imageMedia" :key="media.mission_media_id">
                    <input type="checkbox" v-model="selectedMedia" :value="media.mission_media_id" :checked="true"/>
                    <label>{{ media.media_name }}</label>
                </div>
            ...
            <div class="row">
                ...
                <div v-for="file in docFiles" :key="file.mission_document_id">
                    <input type="checkbox" v-model="selectedDocuments" :value="file.mission_document_id" :checked="true"/>
                    <label>{{ file.document_name }}</label>
                </div>
...
export default {
    ...
    methods: {
        ...
        async submitForm() {
            ...
            this.selectedMedia.forEach(selectedMediaId => {
                this.formData.append('selected_media[]', selectedMediaId);
            });
            this.selectedDocuments.forEach(selectedDocumentId => {
                this.formData.append('selected_documents[]', selectedDocumentId);
            });
            ...
            axios
                .post(`/api/admin/mission/${this.$route.params.id}`, this.formData, {
                    headers: {
    ...
    async created() {
        try {
            ...
            const editMissionResponse = await axios.get(`/api/admin/mission/${this.$route.params.id}`);
            this.mission = editMissionResponse.data;
            this.selectedCountryId = editMissionResponse.data.country_id;
            // this.selectedCityId = editMissionResponse.data.city_id;
            this.mission.start_date = editMissionResponse.data.start_date.slice(0, 10);
            this.mission.end_date = editMissionResponse.data.end_date.slice(0, 10);
            if (editMissionResponse.data.mission_type === "GOAL") {
                this.mission.goal_objective_text = editMissionResponse.data.goal_mission.goal_objective_text;
                this.mission.goal_value = editMissionResponse.data.goal_mission.goal_value;
            }
            if (editMissionResponse.data.mission_type === "TIME") {
                this.mission.total_seats = editMissionResponse.data.time_mission.total_seats;
                this.mission.registration_deadline = editMissionResponse.data.time_mission.registration_deadline.slice(0, 10);
            }
            editMissionResponse.data.mission_skill.forEach((missionSkill) => {
                this.selectedSkills.push(missionSkill.skill_id);
            });
            const youtubeMedia = editMissionResponse.data.mission_media.find(
                (media) => media.media_name === "youtube"
            );

            this.imageMedia = editMissionResponse.data.mission_media.filter(
                (imagemedia) => imagemedia.media_name !== "youtube"

            );
            this.mission.media_names = youtubeMedia.media_path;

            this.docFiles = editMissionResponse.data.mission_document;
            // this.mission. = editMissionResponse.data.title;
            // this.mission.start_date = editMissionResponse.data.start_date.slice(0, 10);


            //console.log(editMissionResponse.data);
            //console.log(this.docFiles[0]);
