      ...
      <table class="table table-responsive border-start border-end">
        ...
        <tbody>
          <tr v-for="story in stories" :key="story.story_id">
            ...
            <td>
                <a :href="getStoryUrl(story.story_id)">
                    <button class="btn btn-outline border px-2 py-1" style="border-radius: 23px;color:#F88634;border-style: solid;border-color: #F88634!important;">
                      View
                    </button>
                  </a>
                <span v-if="story.status === 'PUBLISHED'" class="border px-2 py-1 text-success border-success" style="border-radius: 23px;">
                    PUBLISHED
                  </span>
                  <a v-else @click="publishStory(story.story_id)" style="text-decoration: none;" aria-label="correct">
                    <img alt="correct" src="../../../public/Images/correct-icon.svg" style="width:25px;height:25px;" >
                  </a>
                  <span v-if="story.status === 'DECLINED'" class="border px-2 py-1 text-danger border-danger" style="border-radius: 23px;">
                    DECLINED
                  </span>
                  <a v-else @click="declineStory(story.story_id)" style="text-decoration: none;" id="application_r_{{$mt->story_id}}">
                    <img src="../../../public/Images/cancle-icon.svg" style="width:25px;height:25px;" alt="cancel">
                  </a>
                  <button type="button" @click="deleteStory(story.story_id)" class="btn btn-white">
